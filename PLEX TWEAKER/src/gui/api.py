...
class Api:
    ...
    def get_build_info(self):
        from src.utils.updater import get_current_version, get_release_channel, is_beta_build
        from src.utils.platform_support import capabilities
        platform = capabilities().get('platform', sys.platform)
        return {
            'name': 'Plex',
            ...
            'beta': is_beta_build(),
            'platform': platform,
            'key_required': not is_beta_build(),
            'repository': 'https://github.com/wareethan468-ux/meow-ware-v2',
        }

    # ── Plex Optimizer (real 66mods engine) ───────────────────────────────
    @staticmethod
    def _plex_optimizer_path():
        # Release builds keep the optimizer as a normal companion executable.
        # Do not embed its self-contained .NET image inside Plex: doing so makes
        # static scanners attribute the optimizer host's SetThreadContext import
        # to the main app and forces a one-file build to drop it at runtime.
        if getattr(sys, 'frozen', False):
            companion = Path(sys.executable).resolve().parent / 'optimizer' / 'PlexOptimizer.exe'
            if companion.is_file():
                return companion
        return Path(get_resource_path(os.path.join('vendor', 'plex-optimizer', 'win-x64', 'PlexOptimizer.exe')))

    def get_plex_optimizer_status(self):
        from src.utils.platform_support import IS_WINDOWS
        path = self._plex_optimizer_path()
        running = any(
            (process.info.get('name') or '').lower() == 'plexoptimizer.exe'
            for process in psutil.process_iter(['name'])
        ) if IS_WINDOWS else False
        return {
            'ok': IS_WINDOWS and path.is_file(),
            'available': IS_WINDOWS and path.is_file(),
            'running': running,
            'engine': '66mods Tweaker 2.0',
            'attribution': 'Powered by the MIT-licensed 66mods Tweaker engine',
            'error': None if IS_WINDOWS and path.is_file() else (
                'Plex Optimizer is only available on Windows' if not IS_WINDOWS
                else 'The Plex Optimizer engine is missing from this build'
            ),
        }

    def launch_plex_optimizer(self):
        status = self.get_plex_optimizer_status()
        if not status.get('available'):
            return status
        try:
            path = self._plex_optimizer_path()
            subprocess.Popen([str(path)], cwd=str(path.parent))
            log('[+] Plex Optimizer launched (66mods engine)')
            return {'ok': True, 'message': 'Plex Optimizer opened'}
        except Exception as exc:
            log(f'[-] Plex Optimizer launch failed: {exc}', (255, 100, 100))
            return {'ok': False, 'error': f'Could not open Plex Optimizer: {exc}'}

    # Legacy in-process optimizer methods remain for backward compatibility
    # with older front-end bundles, but the current Plex UI launches the full
    # optimizer engine above.
    _OPTIMIZER_ACTIONS = {
        'game_mode': {'name': 'Windows Game Mode', 'restart': False, 'risk': 'low'},
        'capture_off': {'name': 'Disable background capture', 'restart': False, 'risk': 'low'},
        'animations_off': {'name': 'Reduce Windows animations', 'restart': True, 'risk': 'low'},
        'power_performance': {'name': 'High-performance power plan', 'restart': False, 'risk': 'medium'},
        'flush_dns': {'name': 'Flush DNS cache', 'restart': False, 'risk': 'low'},
        'temp_cleanup': {'name': 'Clean old temporary files', 'restart': False, 'risk': 'medium'},
    }

    def get_optimizer_status(self):
        from src.utils.platform_support import IS_WINDOWS
        if not IS_WINDOWS:
            return {'ok': False, 'error': 'PC Optimizer is only available on Windows'}
        admin = False
        try:
        ...
        try:
            ...
            active_plan = (result.stdout or '').strip() or 'Unknown'
        except Exception:
            pass
        backup_path = Config.APP_DIR / 'optimizer_backup.json'
        return {
            'ok': True,
            'admin': admin,
            'cpu': str(psutil.cpu_count(logical=True) or 0),
            'memory': f"{psutil.virtual_memory().total / (1024 ** 3):.1f} GB",
            'disk_free': f"{psutil.disk_usage(os.environ.get('SystemDrive', 'C:') + os.sep).free / (1024 ** 3):.0f} GB",
            'active_plan': active_plan,
            'restore_available': backup_path.is_file(),
            'actions': [{**value, 'id': key} for key, value in self._OPTIMIZER_ACTIONS.items()],
        }

    @staticmethod
    def _optimizer_registry_specs():
        return {
            'game_mode': [
                ('Software\\Microsoft\\GameBar', 'AllowAutoGameMode', 1, 'dword'),
                ('Software\\Microsoft\\GameBar', 'AutoGameModeEnabled', 1, 'dword'),
            ],
            'capture_off': [
                ('System\\GameConfigStore', 'GameDVR_Enabled', 0, 'dword'),
                ('Software\\Microsoft\\Windows\\CurrentVersion\\GameDVR', 'AppCaptureEnabled', 0, 'dword'),
            ],
            'animations_off': [
                ('Control Panel\\Desktop\\WindowMetrics', 'MinAnimate', '0', 'string'),
                ('Software\\Microsoft\\Windows\\CurrentVersion\\Explorer\\VisualEffects', 'VisualFXSetting', 2, 'dword'),
            ],
        }

    def run_optimizer_actions(self, action_ids):
        from src.utils.platform_support import IS_WINDOWS
        if not IS_WINDOWS:
            return {'ok': False, 'error': 'PC Optimizer is only available on Windows'}
        selected = [str(item) for item in (action_ids or []) if str(item) in self._OPTIMIZER_ACTIONS]
        if not selected:
            return {'ok': False, 'error': 'Select at least one optimization'}
        import winreg
        import tempfile
        import shutil
        backup_path = Config.APP_DIR / 'optimizer_backup.json'
        try:
            ...
            backup = {}
        backup.setdefault('created_at', time.time())
        backup.setdefault('registry', [])
        backup.setdefault('power_scheme', None)
        results = []
        specs = self._optimizer_registry_specs()
        try:
            for action in selected:
                for key_path, value_name, new_value, value_kind in specs.get(action, []):
                    with winreg.CreateKeyEx(winreg.HKEY_CURRENT_USER, key_path, 0, winreg.KEY_READ | winreg.KEY_WRITE) as key:
                        already_saved = any(row.get('path') == key_path and row.get('name') == value_name for row in backup['registry'])
                        if not already_saved:
                            try:
                                old_value, old_type = winreg.QueryValueEx(key, value_name)
                                backup['registry'].append({'path': key_path, 'name': value_name, 'exists': True, 'value': old_value, 'type': old_type})
                            except FileNotFoundError:
                                backup['registry'].append({'path': key_path, 'name': value_name, 'exists': False})
                            backup_path.parent.mkdir(parents=True, exist_ok=True)
                            backup_path.write_text(json.dumps(backup, indent=2), encoding='utf-8')
                        reg_type = winreg.REG_DWORD if value_kind == 'dword' else winreg.REG_SZ
                        winreg.SetValueEx(key, value_name, 0, reg_type, new_value)
                    results.append(f"Applied {self._OPTIMIZER_ACTIONS[action]['name']}")
                if action == 'power_performance':
                    current = subprocess.run(['powercfg', '/getactivescheme'], capture_output=True, text=True, timeout=5, creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0))
                    import re
                    match = re.search(r'[0-9a-fA-F-]{36}', current.stdout or '')
                    if not backup.get('power_scheme'):
                        backup['power_scheme'] = match.group(0) if match else None
                        backup_path.parent.mkdir(parents=True, exist_ok=True)
                        backup_path.write_text(json.dumps(backup, indent=2), encoding='utf-8')
                    done = subprocess.run(['powercfg', '/setactive', 'SCHEME_MIN'], capture_output=True, text=True, timeout=8, creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0))
                    if done.returncode != 0:
                        raise RuntimeError((done.stderr or done.stdout or 'powercfg failed').strip())
                    results.append('Enabled the high-performance power plan')
                elif action == 'flush_dns':
                    done = subprocess.run(['ipconfig', '/flushdns'], capture_output=True, text=True, timeout=10, creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0))
                    if done.returncode != 0:
                        raise RuntimeError((done.stderr or done.stdout or 'DNS flush failed').strip())
                    results.append('Flushed the Windows DNS cache')
                elif action == 'temp_cleanup':
                    removed = 0
                    ...
                    for item in Path(tempfile.gettempdir()).iterdir():
                        try:
                            ...
                            continue
                    results.append(f'Cleaned {removed} old temporary items')
            backup_path.parent.mkdir(parents=True, exist_ok=True)
            backup_path.write_text(json.dumps(backup, indent=2), encoding='utf-8')
            log(f"[+] PC Optimizer applied {len(selected)} selected actions")
            return {'ok': True, 'results': results, 'restart_recommended': 'animations_off' in selected}
        except Exception as exc:
            log(f"[-] PC Optimizer failed: {exc}", (255, 100, 100))
            rollback = self.restore_optimizer_changes()
            return {'ok': False, 'error': str(exc), 'results': results, 'rolled_back': bool(rollback.get('ok'))}

    def restore_optimizer_changes(self):
        from src.utils.platform_support import IS_WINDOWS
        if not IS_WINDOWS:
            return {'ok': False, 'error': 'PC Optimizer is only available on Windows'}
        import winreg
        ...
        if not backup_path.is_file():
            return {'ok': False, 'error': 'No optimizer backup is available'}
        try:
            backup = json.loads(backup_path.read_text(encoding='utf-8'))
            for item in reversed(backup.get('registry', [])):
                with winreg.CreateKeyEx(winreg.HKEY_CURRENT_USER, item['path'], 0, winreg.KEY_WRITE) as key:
                    if item.get('exists'):
                        winreg.SetValueEx(key, item['name'], 0, int(item['type']), item.get('value'))
                    else:
                        try: winreg.DeleteValue(key, item['name'])
                        except FileNotFoundError: pass
            if backup.get('power_scheme'):
                subprocess.run(['powercfg', '/setactive', backup['power_scheme']], timeout=8, creationflags=getattr(subprocess, 'CREATE_NO_WINDOW', 0), check=False)
            backup_path.unlink(missing_ok=True)
            log('[+] PC Optimizer restored the saved Windows settings')
            return {'ok': True, 'message': 'Saved optimizer changes were restored. Temporary-file cleanup cannot be undone.'}
        except Exception as exc:
    ...
    def start_roblox_download(self):
        ...
        if installed and installed == latest:
            try:
                matching_path = next(
                    (path for path in RobloxManager.get_all_roblox_version_dirs()
                     if os.path.basename(os.path.normpath(path)).lower() == latest.lower()),
                    None,
                )
                if matching_path:
                    self.settings['launch_target_path'] = matching_path
                    Config.save_settings(self.settings)
            except Exception:
                pass
            try:
        ...
        versions_root = str(
            self.settings.get('roblox_build_install_root', '')
            or RobloxManager.resolve_download_versions_root()
            or ''
        )
        ...
        def _worker():
            ...
            try:
                ...
                if result.get("ok"):
                    ...
                    if final_path:
                        self.settings['launch_target_path'] = final_path
                        Config.save_settings(self.settings)
    ...
    def get_settings(self):
        ...
        return {
            ...
            'continuous_instance_detection': self.settings.get('continuous_instance_detection', True),
            'theme': self.settings.get('theme', 'dark'),
            'close_to_tray': self.settings.get('close_to_tray', False),
            'start_in_tray': self.settings.get('start_in_tray', False),
            ...
            # Bootstrapper (launcher) appearance preferences — persisted via the
            # allowlisted save_setting() and consumed by the Bootstrapper view's
            # Appearance tab (accent, card density, launch tips).
            'boot_accent': self.settings.get('boot_accent', 'purple'),
            'boot_card_style': self.settings.get('boot_card_style', 'full'),
            'boot_show_tips': self.settings.get('boot_show_tips', True),
        }
    ...
    _SAVE_SETTING_VALIDATORS = {
        ...
        'boot_accent': lambda v: v if v in ('purple', 'green', 'blue', 'amber') else 'purple',
        'boot_card_style': lambda v: v if v in ('full', 'compact') else 'full',
        'boot_show_tips': lambda v: bool(v),
    }
    ...
    def set_auto_apply(self, value):
        ...
        if value:
            # Re-evaluate an already-running client immediately. Without this,
            # a PID observed while Auto Apply was off remained "processed" and
            # was skipped until Roblox restarted.
    ...
    def set_continuous_instance_detection(self, value):
        enabled = bool(value)
        self.settings['continuous_instance_detection'] = enabled
        Config.save_settings(self.settings)
        if enabled:
            # Forget the previous result so an already-running client is
            # handled on the very next monitor tick.
            self.processed_pids.clear()
            self._scheduled_due.clear()
        log(f"[+] Continuous Roblox detection: {'ON' if enabled else 'OFF'}")
        return enabled
    ...
    def set_start_in_tray(self, value):
        """Start minimized to the system tray on next launch.

        Only takes effect at startup and only when the tray icon exists (which
        requires close-to-tray so there is always a way to bring the window
        back). The window is still created visible and hidden AFTER the first
        render — never with hidden=True, which left WebView2 blank/gray."""
        enabled = bool(value)
        self.settings['start_in_tray'] = enabled
        Config.save_settings(self.settings)
        log(f"[+] Start in tray: {'ON' if enabled else 'OFF'}")
        return enabled
    ...
    def set_include_roblox_offsets_in_discovery(self, value):
        """Expose internal Roblox function offsets as reference-only Discovery rows."""
        enabled = bool(value)
        self.settings['include_roblox_offsets_in_discovery'] = enabled
        Config.save_settings(self.settings)
        return {'ok': True, 'enabled': enabled}
    ...
    def get_available_flags(self, search='', offset=0, limit=300):
        ...
        for item in results:
            item['source_category'] = 'fflags_hpp'

        if self.settings.get('include_roblox_offsets_in_discovery', False):
            try:
                from src.core import internal_offsets
                needle = search_lower.strip()
                roblox_rows = []
                for name, address in internal_offsets.get_internal_offsets().items():
                    ...
                    roblox_rows.append({
                        ...
                        'prefix': 'RVA',
                        'match_kind': 'direct',
                        'source_category': 'roblox_offsets',
                        'reference_only': True,
                        'offset_value': f'0x{int(address):X}',
                    })
                roblox_rows.sort(key=lambda item: (0 if search_lower and item['name'].lower().startswith(search_lower) else 1, item['name'].lower()))
                # Reserve room for both categories so one large catalog cannot hide the other.
                if int(offset or 0) == 0 and roblox_rows:
                    split = max(1, int(limit or 300) // 2)
                    results = results[:split] + roblox_rows[:max(0, int(limit or 300) - split)]
            except Exception as exc:
                log(f"[!] Could not add Roblox offsets to Discovery: {exc}", (255, 200, 100))
        return results
    ...
    def get_roblox_build_options(self):
        """Return downloadable builds and the version used by Launch Roblox."""
        from src.core import offset_loader
        from src.core.version_changer import deployment
        installed = RobloxManager.get_roblox_version_string()
        if installed == 'unknown':
            installed = None
        preferred_path = str(self.settings.get('launch_target_path', '') or '')
        return {
            'ok': True,
            'installed_version': installed,
            'latest_production': deployment.get_latest_production_guid(),
            'offset_version': offset_loader.last_source_build(),
            'preferred_path': preferred_path,
            'preferred_version': os.path.basename(os.path.normpath(preferred_path)) if preferred_path else None,
            'install_root': str(self.settings.get('roblox_build_install_root', '') or RobloxManager.resolve_download_versions_root() or ''),
            'versions': offset_loader.fetch_available_versions(80),
        }

    def choose_roblox_install_folder(self):
        """Pick and persist a writable root used for manually downloaded Roblox builds."""
        if not self._window:
            return {'ok': False, 'error': 'Window is not ready'}
        try:
            result = self._window.create_file_dialog(dialog_type=20)
            if not result:
                return {'ok': False, 'cancelled': True}
            path = os.path.abspath(str(result if isinstance(result, str) else result[0]))
            os.makedirs(path, exist_ok=True)
            probe = os.path.join(path, '.vellium-write-test')
            with open(probe, 'w', encoding='utf-8') as handle:
                handle.write('ok')
            os.remove(probe)
            self.settings['roblox_build_install_root'] = path
            Config.save_settings(self.settings)
            return {'ok': True, 'path': path}
        except Exception as exc:
            return {'ok': False, 'error': f'Choose a writable folder: {exc}'}

    def kill_roblox_now(self):
        """Stop running Roblox processes after an explicit user action."""
        try:
            count = self.roblox_manager.kill_roblox() if self.roblox_manager else 0
            return {'ok': True, 'count': int(count or 0), 'message': f'Stopped {int(count or 0)} Roblox process(es).'}
        except Exception as exc:
            return {'ok': False, 'error': str(exc)}
    ...
    def start_roblox_version_download(self, mode='latest', version='', allow_downgrade=False):
        ...
        versions_root = str(self.settings.get('roblox_build_install_root', '') or RobloxManager.resolve_download_versions_root() or '')
        if not versions_root:
            return {'state': 'error', 'message': 'No Roblox install directory found.'}
        try:
            cache_dirs = RobloxManager.get_all_roblox_version_dirs() or []
    ...
    def get_monitor_status(self):
        ...
        if pid:
            try:
                ...
                # A short sampling window produces a real utilization reading.
                # interval=None on a newly-created Process always reports 0.0.
                cpu = proc.cpu_percent(interval=0.12)
    ...
    def _monitor_loop(self):
        ...
        while True:
            try:
                ...
                # A client opened before Vellium may not own the exact window
                # title yet (loading screen, fullscreen, alternate locale).
                # When continuous discovery is enabled, fall back to the real
                # process list and prefer the newest instance. This only finds
                # and attaches; it never starts Roblox.
                if not pid and self.settings.get('continuous_instance_detection', True):
                    instances = self.roblox_manager.list_roblox_processes()
                    if instances:
                        pid = instances[-1]
                ...
                if pid:
                    ...
                    auto_on = (
                        self.settings.get('auto_apply', False)
                        and self.settings.get('continuous_instance_detection', True)
                    )
                    ...
                    if auto_on and new_pid and self.flag_manager.offsets_loaded:
                        ...
                        if self.roblox_manager.attach(pid):
                            # Mark processed only after a successful attach.
                            # Previously a failed first attach permanently
                            # suppressed retries for this PID.
                        ...
                        self.roblox_manager.attach(pid)
                    else:
