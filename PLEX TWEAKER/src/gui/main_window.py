...
class MainWindow:
    def __init__(self):
        ...
        self.window = webview.create_window(
            title='Spotify' if self.api.settings.get('disguise_mode', False) else 'Plex',
            url=ui_path,
            js_api=self.api,
            width=width,
            height=height,
    ...
    def run(self):
        ...
        def on_start(window):
            ...
            # Always remain visible after startup. Older settings files may
            # contain start_in_tray=true; honoring that made the window flash
            # and disappear, which is indistinguishable from a startup crash.
            # Close-to-tray remains available as an explicit user action.
            if self.api.settings.get('start_in_tray', False):
                self.api.settings['start_in_tray'] = False
                try:
                    from src.utils.config import Config
                    Config.save_settings(self.api.settings)
                except Exception:
                    pass
