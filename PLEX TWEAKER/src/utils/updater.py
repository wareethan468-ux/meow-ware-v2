...
def get_release_channel():
    """Return the explicit channel, falling back to PEP 440 version metadata."""
    try:
        from src.utils.helpers import get_resource_path
        with open(get_resource_path(VERSION_FILE), "r", encoding="utf-8") as f:
            data = json.load(f)
        channel = str(data.get("channel", "")).strip().lower()
        if channel:
            return channel
        version = Version(str(data.get("version", "0.0.0")))
        if version.is_devrelease:
            return "dev"
        if version.pre:
            return {"a": "alpha", "b": "beta", "rc": "preview"}.get(version.pre[0], "preview")
        return "stable"
...
def is_beta_build():
    """Use packaging's PEP 440 parser as the source of truth for prereleases."""
    try:
        return Version(get_current_version()).is_prerelease
    except InvalidVersion:
        return get_release_channel() in {"beta", "alpha", "dev", "nightly", "preview"}
