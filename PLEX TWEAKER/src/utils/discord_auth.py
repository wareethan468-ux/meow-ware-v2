...
def get_auth_state() -> Dict[str, Any]:
    ...
    from src.utils.updater import is_beta_build
    beta_build = is_beta_build()
    with _AUTH_LOCK:
        try:
            if AUTH_FILE.exists():
                with open(AUTH_FILE, "r", encoding="utf-8") as f:
                    ...
                    if isinstance(data, dict):
                        ...
                        if beta_build:
                            key_valid = True
                        authenticated = bool(terms_accepted and discord_user and (key_valid or beta_build))
                        return {
