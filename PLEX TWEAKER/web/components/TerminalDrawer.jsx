    ...
    <section className={`terminal-drawer${collapsed ? ' collapsed' : ''}`} aria-label="Toggle terminal">
      ...
      {!collapsed && <>
        <div className="terminal-output" ref={outputRef}>
          {visibleLogs.length ? visibleLogs.map((line, index) => <div key={`${index}-${line}`}>{line}</div>) : <span>{query ? 'No matching output.' : 'Plex ready, waiting for a client.'}</span>}
        </div>
        <footer className="terminal-statusbar">
          <span>Ln {Math.max(visibleLogs.length, 1)}, Col 1</span><span>UTF-8</span><span>Plex</span>
