...
export default function TitleBar({ onOpenCommands }) {
  ...
  return (
    ...
    >
      ...
      <span className="titlebar-name">{disguised ? 'Spotify' : 'Plex'}</span>
      <div className="titlebar-sep" />
      <span className="titlebar-context">{disguised ? 'Music Player' : 'FastFlag Injector'}</span>
      <div className="titlebar-links">
        <button className="titlebar-command-trigger" onClick={onOpenCommands}><Icon name="search" size={12}/><span>Search</span><kbd>Ctrl K</kbd></button>
      ...
      <Modal open={exitOpen} onClose={() => setExitOpen(false)} title="Exit Plex" width="390px"
        footer={<>
        ...
        <p className="modal-body-text">Are you sure you want to close Plex? Active Roblox processes will keep running.</p>
      </Modal>
    </header>
  );
}
