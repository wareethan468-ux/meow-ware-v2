...
export default class AppErrorBoundary extends React.Component {
  ...
  render() {
    ...
    return <main className="crash-recovery"><section><span><Icon name="shield" size={22}/></span><small>ANTI-CRASH RECOVERY</small><h1>Plex recovered the window.</h1><p>{this.state.message}</p><div><button className="btn primary" onClick={this.recover}><Icon name="refresh" size={13}/> Reload safely</button><button className="btn" onClick={()=>callDesktop('open_config_folder')}><Icon name="folder" size={13}/> Open recovery files</button></div><em>The backend remains running. A local report was saved to ui-crash.log.</em></section></main>;
  }
}
