...
export default function OffsetsView({ notify }) {
  ...
  const offsetsMatch = Boolean(data.active_offset_version && data.installed_version && data.active_offset_version === data.installed_version);
  ...
  return (
    <div className="manager-view view">
      ...
      <div className="offset-status-grid">
        <section className={offsetsMatch ? 'healthy' : 'attention'}>
          <div className="offset-card-label"><small>Active offsets</small><VersionBadge kind={offsetsMatch ? 'latest' : 'downgrade'}>{offsetsMatch ? 'Matched' : 'Check'}</VersionBadge></div>
          <strong>{short(data.active_offset_version)}</strong>
          <em>{data.active_offset_version || 'No dump loaded'}</em>
        </section>
        <section>
          <div className="offset-card-label"><small>Installed Roblox</small><VersionBadge kind="installed"/></div>
          <strong>{short(data.installed_version)}</strong>
          <em>{data.installed_version || 'No install detected'}</em>
        </section>
        <section>
          <div className="offset-card-label"><small>Latest production</small><VersionBadge kind="latest"/></div>
          <strong>{short(data.latest_production)}</strong>
          <em>{data.latest_production || 'Checking channel'}</em>
        </section>
      </div>

      <div className={`offset-health-banner ${offsetsMatch ? 'healthy' : 'attention'}`}>
        <span><Icon name={offsetsMatch ? 'check' : 'alert'} size={15}/></span>
        <div><strong>{offsetsMatch ? 'Offsets match your installed Roblox build' : 'Offset version needs attention'}</strong><small>{offsetsMatch ? 'Live FastFlag injection is aligned with this client version.' : 'Sync to the installed build before using live memory features.'}</small></div>
        <button className="btn" onClick={refresh}><Icon name="refresh" size={12}/> Recheck</button>
      </div>
        ...
        <section className="manager-panel">
          ...
          {mode === 'custom' && (
            <div className="offset-custom">
              ...
              <CustomSelect searchable value={version} onChange={setVersion} options={versions} label="Choose a Roblox version" className="offset-version-select" />
            </div>
          )}
      ...
      <div className="offset-help-grid">
        <article><span><Icon name="shield" size={15}/></span><div><strong>Version-safe injection</strong><p>Vellium compares the active dump against your installed client before live writes.</p></div></article>
        <article><span><Icon name="database" size={15}/></span><div><strong>Local cache</strong><p>Previously synced definitions stay available when upstream sources are temporarily offline.</p></div></article>
        <article><span><Icon name="refresh" size={15}/></span><div><strong>One-click recovery</strong><p>Match installed selects the exact Roblox build detected on this computer.</p></div></article>
      </div>
