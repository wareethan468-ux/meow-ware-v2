...
The Discord bot requires `DISCORD_TOKEN` and `SUPABASE_SECRET_KEY` environment variables. Set `DOWNLOAD_URL` to the private release URL shown after a user accepts `/download`; `DOWNLOAD_VERSION` and `DOWNLOAD_TERMS_VERSION` customize the agreement card. Run `bot/setup_database.sql` once in Supabase to create the acceptance-audit table. The desktop license client uses a Supabase publishable key; never place a Supabase secret/service-role key, Discord token, or private download URL in source control.
